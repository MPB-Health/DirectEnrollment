...
> **DirectEnrollment implementation note:** this repo shows a **fixed** product text `"Direct"` (not the product's `name`, which defaults to `"Direct Enrollment"`), so the snapshot is `setListBillSummary(paymentMethod === 'list-bill' ? { productName: 'Direct' } : null)`. The snapshot is applied in **both** success paths in `handleSubmit`: the normal success return **and** the 409 idempotency-recovery return (where the member is recovered via `fetchSubmissionStatus`). The completion sentence keeps the MPB Health wording.
